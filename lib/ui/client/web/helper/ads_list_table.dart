...
class AdsListTable extends ConsumerStatefulWidget {
  final String clientUuid;
  const AdsListTable({Key? key,required this.clientUuid}) : super(key: key);

  @override
  ConsumerState<AdsListTable> createState() => _AdsListTableState();
}

class _AdsListTableState extends ConsumerState<AdsListTable> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    return CommonTableGenerator(
      ...
      headerContent: [
        ...
        CommonHeader(title: LocaleKeys.keyDate.localized, flex: 2),
      ...
      childrenContent: (index) {
        ...
        return [
          ...
          CommonRow(title: item?.adsMediaType ?? '', flex: 2),
          Expanded(
            flex: 2,
            child: Row(
              children: [
      ...
      isSwitchLoading: (index) =>
          clientAdsWatch.changeClientAdsStatusState.isLoading && index == clientAdsWatch.statusTapIndex,
      statusValue: (index) => clientAdsWatch.clientAdsList[index]?.active,
      hideStatus: (index) =>
          (clientAdsWatch.clientAdsList[index]?.status == 'REJECTED') ||
          (clientAdsWatch.clientAdsList[index]?.status == 'PENDING'),
      onStatusTap: (value, index) async {
        clientAdsWatch.updateStatusIndex(index);
        clientAdsWatch.changeClientAdsStatusApi(
          context,
      ...
      isDeleteVisible: (index) => clientAdsWatch.clientAdsList[index]?.active,
      isDeleteLoading: (index) =>
          clientAdsWatch.deleteClientAdsState.isLoading && index == clientAdsWatch.statusTapIndex,
      onDelete: (index) {
        clientAdsWatch.updateStatusIndex(index);
      ...
      isEditVisible: (index) => clientAdsWatch.clientAdsList[index]?.active,
      onEdit: (index) {
        ...
        showCommonWebDialog(
          ...
          width: 0.3,
          context: context,
          dialogBody: EditAdNameDialog(uuid: clientAdsWatch.clientAdsList[index]?.uuid ?? '',clientUuid: widget.clientUuid,), /// passed clientUuid
        );
      },
      ...
      onForwardArrow: (index) {
        ref
            ...
            .push(
              NavigationStackItem.adsDetails(
      ...
      isConfirmationFieldVisible: (index) =>
          (clientAdsWatch.clientAdsList[index]?.status == 'PENDING') &&
          (clientAdsWatch.clientAdsList[index]?.files?.isNotEmpty ?? false),
      isApproveLoading: (index) =>
          clientAdsWatch.acceptRejectAdsState.isLoading && index == clientAdsWatch.statusTapIndex,
      onApprove: (index) async {
        clientAdsWatch.updateStatusIndex(index);
        await clientAdsWatch.acceptRejectAdsApi(
          context,
          uuid: clientAdsWatch.clientAdsList[index]?.uuid ?? '',
          status: 'ACCEPTED',
        );
      ...
      onDecline: (index) {
        ...
        showCommonWebDialog(
          ...
          width: 0.3,
          context: context,
          dialogBody: RejectAdReasonDialog(uuid: clientAdsWatch.clientAdsList[index]?.uuid ?? ''),
        );
      },
